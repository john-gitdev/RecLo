...
```bash
./scripts/measure_cpu_android.sh -p com.reclo.app.dev -n 15 -d 2 -o /tmp/omi_cpu_baseline.csv
```
...
# Examples:
./scripts/measure_cpu_android.sh -p com.reclo.app.dev -n 15 -d 2
./scripts/measure_cpu_android.sh -p com.reclo.app.dev -n 20 -d 1 -o /tmp/omi_cpu_idle.csv
```
...
```bash
# Single sample
adb shell "top -b -n 1 | grep com.reclo.app.dev"

# Continuous monitoring
adb shell "top -d 2 | grep --line-buffered com.reclo.app.dev"
```
...
# 3. Compare
./scripts/compare_cpu_builds.sh /tmp/baseline.apk "baseline" /tmp/feature.apk "feature" -p com.reclo.app.dev
```
